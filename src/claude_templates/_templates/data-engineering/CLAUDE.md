...
2. **Update `project-specific/` as part of every PR** that changes the topic area. If you add a pipeline, update the pipeline docs. If you change a schema, update the data model docs. Documentation updates are not optional — the work is incomplete without them.
...
4. **Keep inventories current.** If a `project-specific/` doc has an inventory table (pipelines, tables, schemas, etc.), it must match the codebase. Update it before starting new work if it's stale.
