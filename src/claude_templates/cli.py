...
@main.command("list-docs")
def list_docs_cmd():
    """List available documentation packs."""
    click.echo()
    for category, items in groupby(DOCUMENTATION.items(), key=lambda x: x[1]["category"]):
        click.echo(f"  {category}")
        for name, meta in items:
            click.echo(f"    {name:<24} {meta['description']}")
        click.echo()
...
@main.command("info-doc")
@click.argument("doc")
def info_doc(doc: str):
    """Show documentation pack details and file list."""
    try:
        meta = DOCUMENTATION[doc]
    except KeyError:
        raise click.ClickException(
            f"Unknown documentation pack: '{doc}'. Run 'claude-templates list-docs' to see options."
        )

    click.echo()
    click.echo(f"  Documentation: {doc}")
    click.echo(f"  Category:      {meta['category']}")
    click.echo(f"  Description:   {meta['description']}")
    click.echo()
    click.echo(f"  {meta['details']}")
    ...
    doc_files = list_documentation_files(doc)
    if doc_files:
        click.echo("  Files:")
        for f in doc_files:
            click.echo(f"    {f}")
...
@click.argument("path", default=".", type=click.Path())
@click.option("--force", is_flag=True, help="Overwrite existing files without prompting.")
def add_doc(doc: str, path: str, force: bool):
    """Add a documentation pack to a project."""
    try:
        doc_dir = get_documentation_dir(doc)
    except KeyError as e:
        raise click.ClickException(str(e))
    ...
    if not force:
        _, existing = diff_template(doc_dir, target)
        if existing:
            click.echo(f"\n  The following files already exist in {target}:")
            for f in existing:
                click.echo(f"    {f}")
    ...
    created, skipped = install_template(doc_dir, target, force=force)
    ...
    if skipped:
        ...
        for f in skipped:
            click.echo(f"    ~ {f}")
    click.echo()
    click.echo(f"  Documentation '{doc}' added successfully.")
    click.echo()
